...
def write_sd(path, sd):
    if isinstance(sd, SecurityDescriptor):
        sd = sd.build()
    elif isinstance(sd, Container):
        sd = SecurityDescriptorStruct.build(sd)
...
SecurityDescriptorStruct = Struct(
    ...
    "sacl"
    / If(
        lambda this: this.header.flags.get("SE_SACL_PRESENT")
        and this.header.sacl_offset >= this._subcons.header.sizeof(),
        Pointer(this.header.sacl_offset, ACLStruct),
    ),
    "dacl"
    / If(
        lambda this: this.header.flags.get("SE_DACL_PRESENT")
        and this.header.dacl_offset >= this._subcons.header.sizeof(),
        Pointer(this.header.dacl_offset, ACLStruct),
    ),
)
...
class SID(Constructable):
    ...
    def __eq__(self, other):
        if isinstance(other, SID):
            return (
                ...
                and self.subauth == other.subauth
            )
    ...
    def __str__(self):
        return "S-" + "-".join(
            str(a) for a in (self.revision, self.auth) + self.subauth
        )
    ...
    def to_construct(self):
        return dict(
            ...
            cnt=len(self.subauth),
            auth=self.auth,
            subauth=self.subauth,
        )
...
class ACE(Constructable):
    ...
    def from_construct(cls, cont):
        ...
        return ACE(
            str(cont.header.type),
            cont.body.access_mask,
            SID.from_construct(cont.body.sid),
    ...
    def to_construct(self):
        ...
        r.body = Container(
            ...
            sid=self.sid.to_construct(),
            flags={f: True for f in self.flags},
        )
        return r
    ...
    def make_inherited(self):
        return ACE(
            ...
            self.access_mask,
            self.sid,
            self.flags | {"INHERITED_ACE"} - {"INHERIT_ONLY_ACE"},
        )
...
class SecurityDescriptor(Constructable):
    ...
    def __init__(
        self,
        *,
        owner_sid=SID_ADMINISTRATORS,
        group_sid=SID_ADMINISTRATORS,
        dacl=[],
        sacl=None,
        sacl_inherit=True,
        dacl_inherit=True,
    ...
    def __repr__(self):
        ...
        if not self.sacl_inherit:
            r.append("    sacl_inherit=False,")
        if not self.dacl_inherit:
            r.append("    dacl_inherit=False,")
        r.append(")")
        return "\n".join(r)
    ...
    def acl_from_construct(cls, cont):
        if cont:
            return [ACE.from_construct(itm) for itm in cont["items"]]
        else:
            return None
    ...
    def to_construct(self):
        ...
        if self.sacl is not None:
            flags |= {"SE_SACL_PRESENT", "SE_SACL_AUTO_INHERITED"}
        if self.dacl is not None:
            flags |= {"SE_DACL_PRESENT", "SE_DACL_AUTO_INHERITED"}
        if not self.sacl_inherit:
        ...
        if r.sacl:
            hdr.sacl_offset = add_element(ACLStruct, r.sacl)
        else:
            hdr.sacl_offset = 0
        if r.dacl:
            hdr.dacl_offset = add_element(ACLStruct, r.dacl)
        else:
            hdr.dacl_offset = 0
        return r


def apply_sd_recursively(
    path, sd=None, *, dacl=None, skip_protected=True, set_owner=False, set_group=False
):
    path = Path(path)
    if sd is None:
    ...
    def visit(p):
        ...
        acl = [
            ace.make_inherited()
            for ace in sd.dacl
            if f"{kind}_INHERIT_ACE" in ace.flags
        ]
        child_sd = SecurityDescriptor.parse(dump_sd(p))
        if (not child_sd.dacl_inherit) and skip_protected:
            return  # do not modify ACL and do not recurse
        child_sd.dacl = acl
        if set_owner:
            child_sd.owner_sid = sd.owner_sid
        if set_group:
            child_sd.group_sid = sd.group_sid
        print(p, child_sd)
        write_sd(p, child_sd)
        if is_dir:
